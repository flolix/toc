...
void printusage() {
    ...
    puts("-----------------------------------------------------------------");
    puts("Invoke:  toc OSC-command   or   toc DO-SCRIPT");
...
void printhelp() {
   setcolor(YELLOW);
    printf("  toc ");
    setcolor(NOCOLOR);
    puts(" - a thin osc client");
    puts("which formats and send OSC messages via UDP");
    puts("-------------------------------------------");
    puts("Invoke:  toc OSC-command  or  toc DO-SCRIPT");
    puts("DO-SCRIPT is usually a chain of COMMAND VALUE pairs. COMMAND could be");
    puts(" -o       .. VALUE is an OSC command");
    ...
    puts("OSC commands");
    puts("------------");
    puts("/foo 1 test 3.14 will send an OSC message with an int, a string and a float.");
    ...
    puts("toc /thatsfast -p 100 -r .. will send a message, wait for 100ms and repeat that");
